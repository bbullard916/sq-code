...
namespace BLINK.RPGBuilder.UIElements
{
    public class ItemSlotHolder : MonoBehaviour, IPointerClickHandler, IDragHandler, IEndDragHandler, IBeginDragHandler, IDropHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private CustomInventoryPanel _CustomInvPanel;
        ...
        public void InitSlot(RPGItem item, int bag_index, int slot_index)
        {
            ...
            useButton = GameObject.Find("Inventory-Use-Button");
        ...
        {
            if (eventData.button == PointerEventData.InputButton.Left && UIEvents.Instance.IsPanelOpen("Inventory"))
            {
                ClearAllSelectedSlots();
                Debug.Log("CLICK " + slotIndex);
                _PlaySound.PlaySoundEffect("ClickSound1");
                Image image = this.GetComponent<Image>();
                image.color = new Color(image.color.r, image.color.g, image.color.b, 1f);
                selectSlotIndex = slotIndex;
                _CustomInvPanel = GameObject.Find("Inventory").GetComponent<CustomInventoryPanel>();
                _CustomInvPanel.slotIndex = selectSlotIndex;
                _CustomInvPanel.thisItem = thisItem;
                _CustomInvPanel.bagIndex = bagIndex;
                return;
            }
            ...
            useButton.GetComponent<CanvasGroup>().alpha = 0;
            useButton.GetComponent<CanvasGroup>().interactable = false;
        }

        private void ClearAllSelectedSlots()
        {
            GameObject[] selectImage;
            selectImage = GameObject.FindGameObjectsWithTag("itemslot-inventory");
            useButton.GetComponent<CanvasGroup>().alpha = 1;
            useButton.GetComponent<CanvasGroup>().interactable = true;
            for (int i = 0; i < selectImage.Length; i++)
            {
                Image image = selectImage[i].GetComponent<Image>();
