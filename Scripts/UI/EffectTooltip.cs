...
namespace BLINK.RPGBuilder.UI
{
    public class EffectTooltip : MonoBehaviour
    {
        ...
        {
            UIEvents.ShowEffectTooltip += ShowEffectTooltip;
            UIEvents.ShowBonusTooltip += ShowBonus;
            UIEvents.HideEffectTooltip += Hide;
        }
        ...
        private void ShowEffectTooltip(CombatEntity entity, RPGEffect effect, RPGEffect.RPGEffectRankData rank)
        {
            ...
            GenerateTooltip(entity, effect, rank);
        }

        private void GenerateTooltip(CombatEntity entity, RPGEffect effect, RPGEffect.RPGEffectRankData rank)
        {
            ClearAllAbilityTooltipElements();

            string description = GenerateBaseTooltipText(entity, effect, rank);
            if(effect.effectType.ToString() == "HealOverTime")
            {
                description = "<color=green>Heals</color><color=orange> " + rank.Damage + " </color><color=white>health over</color> " + "<color=orange>" + effect.duration.ToString() + "</color><color=white> seconds";
            }
            if (effect.effectType.ToString() == "Stat")
            {
                foreach (var t in entity.GetStats())
                {
                    string[] g = t.Value.stat.ToString().Split('_');
                    if (effect.entryDisplayName.Contains(g[0]))
                    {

                        float value = CombatUtilities.GetCurrentStatValue(entity, t.Value.stat.ID);
                        foreach (var t1 in t.Value.stat.statBonuses)
                        {
                            description = "<color=green>" + g[0] + "</color><color=white> increased to </color><color=orange> " + value + " </color><color=white>for </color> " + "<color=orange>" + effect.duration.ToString() + "</color><color=white> seconds";
                        }

                    }
                }        
            }
            //foreach (var effectApplied in rank.nestedEffects)
            //{
            //    string newTooltipText = "";
            //    if (texts >= 1) newTooltipText += " ";
            //    newTooltipText += GenerateEffectAppliedTooltipText(entity, effect, effectApplied);
        ...
        private string GenerateBaseTooltipText(CombatEntity entity, RPGEffect effect, RPGEffect.RPGEffectRankData rank)
        {
        ...
        private string GenerateEffectAppliedTooltipText(CombatEntity entity, RPGEffect effect, RPGAbility.AbilityEffectsApplied effectApplied)
        {
            ...
            {
                fullText = fullText.Replace("["+ keyword + "]", ConvertKeywordToDescription(entity, effect, effectApplied, keyword));
            }
            return fullText;
        }
        ...
        private string ConvertKeywordToDescription(CombatEntity entity, RPGEffect effect, RPGAbility.AbilityEffectsApplied effectApplied, string keyword)
        {
            string text = "";
            effect = GameDatabase.Instance.GetEffects()[effectApplied.effectID];
