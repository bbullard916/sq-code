...
namespace BLINK.RPGBuilder.Managers
{
    public class CombatManager : MonoBehaviour
    {
        ...
        public bool UseRequirementsMet(CombatEntity casterInfo, CombatEntity targetInfo, RPGAbility ability, RPGAbility.RPGAbilityRankData rankREF, bool abMustBeKnown, bool isToggleTick = false)
        {
            ...
            if (!isToggleTick && casterInfo.IsPlayer() && !rankREF.CanUseDuringGCD && rankREF.isGCD && currentGCD > 0)
            {
        ...
        public void InitAbility(CombatEntity casterEntity, RPGAbility thisAbility, RPGAbility.RPGAbilityRankData rank, bool abilityKnown)
        {
            ...
            {
                casterEntity.EndAbility(0);
                SimpleCustomAlrtUI.Instance.SendCustomMessage("Not enough Action Points", "red", new Vector3(0,0.3f,0));
                return;
            }
        ...
        {
            Debug.Log("target" + targetEntity.name);
            if (!targetEntity.IsDead() && !targetEntity.IsPlayer())
                StartCoroutine(targetEntity.transform.GetComponentInChildren<NpcEventManager>().handleHightlight());
            if (!targetEntity.IsDead() && targetEntity.IsPlayer())
                StartCoroutine(targetEntity.transform.GetComponentInChildren<PlayerEventManager>().handleHightlight());
            if (targetEntity.IsDead()) yield break;
            ...
            switch (effect.effectType)
            {
                ...
                case RPGEffect.EFFECT_TYPE.RollLootTable:
                    ...
                    if (lootTableREF.UseRequirementsTemplate)
                    {
                        if (lootTableREF.RequirementsTemplate != null && !RequirementsManager.Instance.RequirementsMet(GameState.playerEntity, lootTableREF.RequirementsTemplate.Requirements).Result) yield break;
                    }
                    if (lootTableREF.Requirements != null && !RequirementsManager.Instance.RequirementsMet(GameState.playerEntity, lootTableREF.Requirements).Result) yield break;
                    //END MY CODE FIX
        ...
        {
            Debug.Log("Handle Effetc" + casterInfo.name);
