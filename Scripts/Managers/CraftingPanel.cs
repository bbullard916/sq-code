...
namespace BLINK.RPGBuilder.Managers
{
    public class CraftingPanel : DisplayPanel
    {
        ...
        private void InitCraftingPanel(RPGCraftingStation station)
        {
            ...
            for (int i = 0; i < curRecipeSlots.Count; i++)
            {
                CraftingRecipeSlotHolder _CraftingItemSlotHolder = curRecipeSlots[i].GetComponent<CraftingRecipeSlotHolder>();
                if (i == 0)
                {
                    curRecipeSlots[i].GetComponent<Button>().Select();
                    _CraftingItemSlotHolder.SelectRecipe();
                    _CraftingItemSlotHolder.SetOutline(true);
                }
                else
                {
                    
                    _CraftingItemSlotHolder.SetOutline(false);
                }    
            }
        }
