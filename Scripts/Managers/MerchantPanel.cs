    ...
    public class MerchantPanel : DisplayPanel
    {
        ...
        private void InitializeMerchantPanel(RPGMerchantTable merchantTable)
        {
            ...
            foreach (var t in merchantTable.onSaleItems)
            {
                ...
                if (count == 0)
                {
                    holder.SetOutline(true);
                }
                else
                {
                    holder.SetOutline(false);
                }
        ...
        public void ClearMerchatOutline()
        {
            foreach (var t in currentMerchantItemSlots)
                if (t.GetComponent<MerchantItemSlotHolder>())
                {
                    t.GetComponent<MerchantItemSlotHolder>().SetOutline(false);
                    t.GetComponent<MerchantItemSlotHolder>().isSelected = false;
                } 
        }
        public void Show(CombatEntity merchantEntity, RPGMerchantTable merchantTable)
        {
            ...
            RPGCurrency currency = GetCurrency("Gold Coin");
            CurrencyText.text = Character.Instance.getCurrencyAmount(currency).ToString();
        }
