        ...
        foreach (var gameAction in gameActionsList)
        {
            ...
            switch (gameAction.type)
            {
                ...
                case GameActionsData.GameActionType.ResetBlocking:
                    ...
                    break;
                case GameActionsData.GameActionType.SaveCustomData:
                    {
                        if(gameAction.CustomeSaveTypeValue == 0)
                        {
                            CustomDataProvider.Instance.SaveStringData(gameAction.stringValue1, gameAction.stringValue2, Character.Instance.CharacterData.CharacterName);
                        }
                        else if (gameAction.CustomeSaveTypeValue == 1)
                        {

                        }
                        else if (gameAction.CustomeSaveTypeValue == 2)
                        {
                            CustomDataProvider.Instance.SaveBoolData(gameAction.stringValue1, bool.Parse(gameAction.stringValue2), Character.Instance.CharacterData.CharacterName);
                        }
