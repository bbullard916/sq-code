...
namespace BLINK.RPGBuilder.WorldPersistence
{
    public class PersistenceManager : MonoBehaviour
    {
        ...
        public bool InteractableObjectListContainsIdentifier(string identifier)
        {
            ...
            if (characterData.GameScenes == null)
            {
                Debug.LogWarning("GameScenes list is null");
                return false;
            }

            int sceneIndex = characterData.GameSceneEntryIndex;

            if (sceneIndex < 0 || sceneIndex >= characterData.GameScenes.Count)
            {
                Debug.LogWarning(
                    $"GameSceneEntryIndex out of range: {sceneIndex}. GameScenes count: {characterData.GameScenes.Count}"
                );
                return false;
            }

            var sceneData = characterData.GameScenes[sceneIndex];

            if (sceneData.SavedInteractableObjects == null)
            {
                Debug.LogWarning("SavedInteractableObjects list is null");
                return false;
            }

            foreach (var saver in sceneData.SavedInteractableObjects)
            {
                if (saver.indentifier == identifier)
                {
                    return true;
                }
            }
