...
namespace BLINK.RPGBuilder.World
{
    public class InteractableObject : MonoBehaviour, IPlayerInteractable
    {
            ...
            foreach (var action in Actions)
            {
                ...
                switch (action.type)
                {
                    ...
                    case InteractableObjectData.InteractableObjectActionType.Chest:
                        //MY CODE FIX
                        if (action.LootTable.UseRequirementsTemplate)
                        {
                            if (action.LootTable.RequirementsTemplate != null && !RequirementsManager.Instance.RequirementsMet(GameState.playerEntity, action.LootTable.RequirementsTemplate.Requirements).Result) continue;
                        }
                        if (action.LootTable.Requirements != null && !RequirementsManager.Instance.RequirementsMet(GameState.playerEntity, action.LootTable.Requirements).Result) continue;
                        //END MY CODE FIX
        ...
        private void OnMouseOver()
        {
            ...
            if (State == InteractableObjectData.InteractableObjectState.Ready && Input.GetMouseButtonUp(1) && !GameState.playerEntity.IsInteractingWithObject())
            {
                if (Vector3.Distance(transform.position, GameState.playerEntity.transform.position) <= MaxDistance)
                {
                    ...
                    {
                        UIInfoWindow.Instance.SendGeneralMessage("You cannot interact at this time");
                        SimpleCustomAlrtUI.Instance.SendCustomMessage("You cannot interact at this time", "green");
                    }
                }
                else
                {
        ...
        public void ShowInteractableUI()
        {
            ...
            else if(RequirementsTemplate == null)
            {
                var pos = transform;
                Vector3 worldPos = new Vector3(pos.position.x, pos.position.y + UIOffsetY, pos.position.z);
                var screenPos = Camera.main.WorldToScreenPoint(worldPos);
