...
{
    public class CustomTargetInfoDisplayManager : MonoBehaviour
    {
        ...
        [SerializeField] public GameObject[] portraitOutline;
        ...
        public void InitTargetUI()
        {
            ...
            {
                statDisplay.Bars[0].GetComponent<CanvasGroup>().alpha = 1;
                statDisplay.Bars[1].GetComponent<CanvasGroup>().alpha = 1;
            }
            if (GameState.playerEntity.GetTarget().IsPlayer())
            {
                ...
                portraitOutline[0].SetActive(false);
            }
            else
            {
                ...
                if (GameState.playerEntity.GetTarget().gameObject.tag == "enemy-boss")
                {
                ...
                portraitOutline[0].SetActive(false);
                portraitOutline[1].SetActive(true);
                if(GameState.playerEntity.GetTarget().tag.Contains("enemy"))
                {
                    portraitOutline[1].GetComponent<Image>().color = Color.red;
                }
                if (GameState.playerEntity.GetTarget().tag.Contains("friendly"))
                {
                    portraitOutline[1].GetComponent<Image>().color = Color.green;
                    foreach (var statDisplay in TargetStatDisplays.Where(statDisplay => statDisplay.statObserved != null))
                    {
                        statDisplay.Bars[0].GetComponent<CanvasGroup>().alpha = 0;
                        statDisplay.Bars[1].GetComponent<CanvasGroup>().alpha = 0;
                    }
                }
            }
            InitAllStatDisplays();
        ...
        private void DisableStatBars()
        {

        }

        private void EnableStatBars()
        {
