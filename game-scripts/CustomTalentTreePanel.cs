...
{
    [Serializable]
    public class CustomTreeNodeSlotDATA
    {
    ...
    [Serializable]
    public class CustomTREE_UI_DATA
    {
    ...
    public enum CustomTalentTreePreviousMenu
    {
        CharacterPanel,
        SkillBook,
        WeaponTemplates
    ...
    public class CustomTalentTreePanel : DisplayPanel
    {
        ...
        public void SetTalentTree(int ID)
        {
            if (TreeNameText.text.Contains("Mining"))
                InitSlot(GameDatabase.Instance.GetTalentTrees()[ID]);
            //Debug.Log("Set ID: " + ID);
            else if (ID == 3 && GameDatabase.Instance.GetClasses()[Character.Instance.CharacterData.ClassID].entryDisplayName == "Mage")
            {
                ID = 6;
                InitSlot(GameDatabase.Instance.GetTalentTrees()[ID]);
                TabMenu[1].SetActive(false);
                TabMenu[2].SetActive(false);
            }
            else if (ID == 3 && GameDatabase.Instance.GetClasses()[Character.Instance.CharacterData.ClassID].entryDisplayName != "Mage")
            {
                InitSlot(GameDatabase.Instance.GetTalentTrees()[ID]);
        ...
        {
            ClearAllTiersData();
            RPGTalentTree thisTree = cbtTree;
            InitTree(thisTree);
        }
        private void ShowAbilityRequirements(RPGAbility ab, RPGTalentTree tree)
        {
            ...
            bool isKnown = CombatUtilities.IsAbilityKnown(ab.ID);

            if (isKnown && curRank >= ab.ranks.Count - 1) return;

            int rankToCheck = isKnown ? curRank + 1 : 0;

            if (rankToCheck < 0 || rankToCheck >= ab.ranks.Count) return;

            var rankREF = ab.ranks[rankToCheck];

            var requirements = "";
            if (rankREF.unlockCost > 0)
            {
                var color = Character.Instance.getTreePointsAmountByPoint(tree.treePointAcceptedID) < rankREF.unlockCost
                    ? "<color=red>"
                    : "<color=green>";
            ...
            RequirementHandling(
                tree,
                RPGBuilderUtilities.getAbilityIndexInTree(ab, tree),
                RPGBuilderUtilities.getTreeIndex(tree),
                requirements
            );
        }
        ...
        public void SwitchTabs(string val)
        {
            if (val == "on")
            {
                TabMenu[0].SetActive(true);
                TabMenu[1].SetActive(true);
                TabMenu[2].SetActive(true);
            }
            else if (val == "off")
            {
                TabMenu[0].SetActive(false);
                TabMenu[1].SetActive(false);    
                TabMenu[2].SetActive(false);
            }
        }
        private void ShowBonusRequirements(RPGBonus bonus, RPGTalentTree tree)
        {
            ...
            bool isKnown = RPGBuilderUtilities.isBonusKnown(bonus.ID);

            if (isKnown && curRank >= bonus.ranks.Count - 1) return;

            int rankToCheck = isKnown ? curRank + 1 : 0;

            if (rankToCheck < 0 || rankToCheck >= bonus.ranks.Count) return;

            var rankREF = bonus.ranks[rankToCheck];
            ...
            RequirementHandling(
                tree,
                RPGBuilderUtilities.getBonusIndexInTree(bonus, tree),
                RPGBuilderUtilities.getTreeIndex(tree),
                requirements
            );
        }
        ...
        public void InitTree(RPGTalentTree tree)
        {
            ...
            Show();
            ClearAllTiersData();
            //Debug.Log("Talent: " + tree.name);
            if (tree.name == "Mining_TALENT_TREE")
            {
                SwitchTabs("off");
                TreeNameText.text = tree.entryDisplayName + " Abilities";
            }
            else
            {
                SwitchTabs("on");
